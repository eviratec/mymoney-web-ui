...
function LogbookPageController($api, $scope, $state, $mdDialog, $timeout, logbook) {
  ...
  $logbookPage.createTransaction = function ($event) {
    ...
    var confirm = $mdDialog.prompt().title('Transaction Summary').placeholder('Groceries ...').ariaLabel('Summary').initialValue('').targetEvent($event).ok('Create Transaction').cancel('Cancel');
